...
export default function Mint() {
  ...
  async function mint() {
    ...
    web3.eth.getTransactionReceipt(tx.hash).then(function (data) {
      ...
      setOpenSea(
        `${OPENSEA_LINK}/${CONTRACT_ADDRESS}/${web3.utils.hexToNumber(
          logs[0].topics[3]
        )}`
      );
    });
      ...
      <div className="flex flex-col items-center justify-evenly">
        {isAuthenticated ? (
          !minting ? (
            <button
              onClick={mint}
              className="text-white bg-purple-700 hover:bg-purple-800 focus:ring-4 focus:ring-purple-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center mb-2 mt-20 dark:bg-purple-600 dark:hover:bg-purple-700 dark:focus:ring-purple-900"
            >
              Mint NFT
            </button>
          ) : (
